#!/usr/bin/env bash
crearUserRepo() {
    # Crea usuario 'repo' sin chroot para servir el repositorio
    if id "repo" &>/dev/null; then
        echo "El usuario repo ya existe."
        return 0
    fi

    sudo useradd repo -d /srv/ftp/repo -s /bin/false
    sudo mkdir -p /srv/ftp/repo
    sudo chown repo:repo /srv/ftp/repo
    sudo chmod 755 /srv/ftp/repo
    sudo passwd repo

    # Agregar al chroot_list para que NO esté enjaulado
    if ! grep -q "^repo$" /etc/vsftpd.chroot_list; then
        echo "repo" | sudo tee -a /etc/vsftpd.chroot_list >/dev/null
    fi

    sudo service vsftpd restart
    echo "[OK] Usuario repo creado en /srv/ftp/repo"
}
setGrupos() {
  ###CREACION DE GRUPOS
